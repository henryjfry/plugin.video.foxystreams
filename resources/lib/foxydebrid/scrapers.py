...
class TorrentApi(Scraper):
    ...
    def find_magnets(self, query=None, tv=False, movie=False, **kwargs):
        #xbmc.log(str(kwargs['id'])+'===>PHIL', level=xbmc.LOGINFO)
        #xbmc.log(str(query)+'===>PHIL', level=xbmc.LOGINFO)
        """Returns iterable of tuples (name, magnet_uri)."""
        ...
        if movie:
            ...
            result = self.api_get(mode='search', search_tvdb=kwargs['id'],
                                  ...
                                  ranked=ranked)
            #xbmc.log(str('tv')+'===>PHIL', level=xbmc.LOGINFO)
            if not result.get('torrent_results'):
                #xbmc.log(str('QUERY')+'===>PHIL', level=xbmc.LOGINFO)
                kwargs['showname'] = re.sub(r'[^A-Za-z0-9 ]+', '', kwargs['showname'])
                query2 = '{} {}'.format(kwargs['showname'], query)
                #xbmc.log(str(query2)+'===>PHIL', level=xbmc.LOGINFO)
                result = self.api_get(mode='search', search_string=query2,
                                      category=category, ranked=ranked)
        elif query:
...
class BitLord(Scraper):
    ...
    def find_magnets(self, query=None, tv=False, movie=False, **kwargs):
        ...
        kwargs['showname'] = re.sub(r'[^A-Za-z0-9 ]+', '', kwargs['showname'])
        #xbmc.log(str(query)+'===>PHIL', level=xbmc.LOGINFO)
        #xbmc.log(str(kwargs['showname'])+'===>PHIL', level=xbmc.LOGINFO)
        if movie:
